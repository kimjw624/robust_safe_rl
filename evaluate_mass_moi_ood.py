...
def collect_parameter_dataset(
    *,
    ...
    The true and nominal systems receive exactly the same commanded action.
    The nominal system always uses nominal mass/inertia and no external force.
    The true system uses the supplied ID external force and parameter scales.
    ...
    for episode_id in range(num_episodes):
        ...
        nominal_state = nominal_dyn.reset(x=d0["x"], v=d0["v"], external_force=np.zeros(3))
        controller.reset()
...
def binary_curves(id_errors, ood_errors):
    ...
    # np.trapezoid was introduced in newer NumPy versions.
    # np.trapz provides the same trapezoidal integration on older releases.
