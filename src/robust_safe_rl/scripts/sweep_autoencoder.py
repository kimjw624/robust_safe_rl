"""Grid-sweep autoencoder OOD detectors, with optional latent regularization.

Trains a grid over architecture / optimization hyperparameters and, optionally,
sparse (L1 activity) or contractive (Jacobian) latent penalties. One shared
ID train/val set and one strict-OOD eval set are collected up front, then reused
across every config. Per-run checkpoints, configs, metrics, history CSVs, plots,
and a master summary are written under ``--sweep_dir``.

Set ``--regularizers none`` (the default) to reproduce a plain unregularized
architecture/latent-dim sweep.
...
from robust_safe_rl.ood.ood_autoencoder.regularization import regularization_penalty
